...
from src.agents import ChatGPT_SAPARAgent_ForAvalon, ChatGPT_CGAgent, ChatGLM_CodeActAgent
...
def load_llama_model(llama_dir):
    ...
    config = AutoConfig.from_pretrained(llama_dir, trust_remote_code=False)
    tokenizer = AutoTokenizer.from_pretrained(llama_dir, trust_remote_code=False)
    model = AutoModelForCausalLM.from_pretrained(llama_dir,
                                                 torch_dtype='auto', ignore_mismatched_sizes=True,
                                                 trust_remote_code=False, config=config, local_files_only=True).half().to("cuda")
    model = model.eval()
...
abs_llama_model_dir = f"/home/hdd/lijinyi/CompressionInAvalon/src/models/{model_name}"
llama_model, llama_tokenizer, _ = load_llama_model(abs_model_dir)
    ...
    for i in range(player_nums):
        ...
        player_args.append(
            (
                ...
                #                   "output_dir": log_dir}
                ChatGLM_CodeActAgent, {"name": name, "role": role, "rule_role_prompt": rule_role_prompt,
